...
namespace Nomad3D
{
	...
	public:
		...
		CQuaternion operator*(const CVector3& rVec) const;
		//Multiply the quaternion by a vector, store the result
		void operator*=(const CVector3& rVec);
		...
		void operator=(const CQuaternion& rQuat);
		//Check for equality
		bool operator==(const CQuaternion& rQuat) const;
	...
	inline CQuaternion SLERP(CQuaternion& rQuat0, CQuaternion& rQuat1, float fInterp)
	{
		...
		return (rQuat0 * SinR(fTheta * (1 - fInterp)) + rQuat1 * SinR(fTheta * fInterp))/SinR(fTheta);
	}
