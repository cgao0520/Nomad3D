...
#define PIXEL_FORMAT555

enum PIXEL_FORMAT_16BIT//针对16bit颜色的像素格式
{
	DIB_PIXEL_FORMAT16_RGB=0,
	DIB_PIXEL_FORMAT16_555=15,
	DIB_PIXEL_FORMAT16_565=16
};
...
public:
	...
	BOOL Create(int width,int height, int bits=24,PIXEL_FORMAT_16BIT format=DIB_PIXEL_FORMAT16_RGB);
	...
	void BitBlt(HDC hDest, int nXDest, int nYDest, int nWidth, int nHeight,
		int xSrc, int ySrc);
	void BitBlt(int nXDest, int nYDest, int nWidth, int nHeight, CDIB& dibSrc,
		int nSrcX, int nSrcY, BYTE *colors=NULL);
	void StretchBlt(HDC hDest, int nXDest, int nYDest, int nDWidth, int nDHeight,
		int xSrc, int ySrc, int  nSWidth, int nSHeight);
	void StretchBlt(int nXDest, int nYDest, int nDWidth, int nDHeight, CDIB& dibSrc,
		int xSrc, int ySrc, int  nSWidth, int nSHeight);
	void ExpandBlt(int nXDest, int nYDest, int xRatio, int yRatio, CDIB& dibSrc, 
		int xSrc, int ySrc, int  nSWidth, int nSHeight);
...
//Get Bits add by c_gao
	unsigned char* GetBits();
protected :
	HANDLE DIBHandle();
	void CreateGammaCurve();
	void Expand(int nXDest, int nYDest, int xRatio, int yRatio, CDIB& dibSrc,
		int xSrc, int ySrc, int  nSWidth, int nSHeight);
	...
	//added by c_gao
	PIXEL_FORMAT_16BIT	m_pixel_format;
};
