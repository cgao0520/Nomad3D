...
CDIB::CDIB(HANDLE hDib,int nBits)
{
	...
	m_pixel_format=DIB_PIXEL_FORMAT16_RGB;
}
...
//modified by c_gao: added pixel format support
BOOL CDIB::Create(int width, int height, int bits,PIXEL_FORMAT_16BIT format)
{
	// Free existing image
	DestroyDIB();
//	ASSERT(bits == 24 || bits == 8);
	...
	if(bits==16 && format!=DIB_PIXEL_FORMAT16_RGB)
		bmInfo.biCompression = BI_BITFIELDS;
	else
...
/*
BOOL CDIB::Create16(int width,int height,int bits,PIXEL_FORMAT_16BIT format)
{
	// Free existing image
	DestroyDIB();
...
BOOL CDIB::Create(BITMAPINFOHEADER& bmInfo)
{
	...
	height = abs(bmInfo.biHeight);//modified by c_gao(add abs macro)
...
//added pixel format support
	...
	{
	case DIB_PIXEL_FORMAT16_565:
		mask[0]=0xf800;
		mask[1]=0x07e0;
		mask[2]=0x001f;
		memcpy(m_pRGB,mask,sizeof(DWORD)*3);
		break;
	case DIB_PIXEL_FORMAT16_555:
		mask[0]=0x7c00;
		mask[1]=0x03e0;
		mask[2]=0x001f;
		memcpy(m_pRGB,mask,sizeof(DWORD)*3);
...
{
	if(m_pixel_format == DIB_PIXEL_FORMAT16_565 || m_pixel_format == DIB_PIXEL_FORMAT16_555)
		return 3;
...
void CDIB::BitBlt(HDC hDest,int nXDest,int nYDest,int nWidth,
				  int nHeight,int xSrc,int ySrc)
{
	SetDIBitsToDevice(hDest,nXDest,nYDest,nWidth,nHeight,xSrc,
		Height()-ySrc-nHeight,0,Height(),m_pBits,m_pInfo,DIB_RGB_COLORS);
}

void CDIB::StretchBlt(HDC hDest,int nXDest,int nYDest,int nDWidth,
					  int nDHeight,int xSrc,int ySrc,int  nSWidth,int nSHeight)
{
	int err;
	err = StretchDIBits(hDest,nXDest,nYDest,nDWidth,nDHeight,xSrc,ySrc,
		nSWidth,nSHeight,m_pBits,(CONST BITMAPINFO * )&m_pInfo->bmiHeader,DIB_RGB_COLORS,SRCCOPY);
}

void CDIB::ExpandBlt(int nXDest,int nYDest,int xRatio,int yRatio,
					 CDIB& dibSrc,int xSrc,int ySrc,int  nSWidth,int nSHeight)
{
...
void CDIB::Expand(int nXDest,int nYDest,int xRatio,int yRatio,
				  CDIB& dibSrc,int xSrc,int ySrc,int  nSWidth,int nSHeight)
{
...
void CDIB::StretchBlt(int nXDest, int nYDest, int nDWidth, int nDHeight,
					  CDIB& dibSrc, int xSrc, int ySrc, int nSWidth, int nSHeight)
{
	SetPalette(dibSrc.m_pRGB);
	nDWidth = nXDest+nDWidth > width ? width-nXDest : nDWidth ; 
...
void CDIB::BitBlt(int nXDest,int nYDest,int nWidth,int nHeight,
				  CDIB& dibSrc,int nSrcX,int nSrcY,BYTE *colors)
{
	SetPalette(dibSrc.m_pRGB);
	if(nXDest < 0)
	{
...
void CDIB::CreateGammaCurve()
{
	...
	for(i=0;i<256;++i)
	{
	    Gamma[i] = (UCHAR)(int)(255 * pow((double)i/255,(double)m_fGamma) + (double)0.5);
	}
}
...
BOOL CDIB::SwitchFromOne(CDIB& dib)
{
	...
	cols[0] = (UCHAR)ClosestColor(dib.m_pRGB);
	cols[1] = (UCHAR)ClosestColor(dib.m_pRGB+1);
	for (i = 0; i < h; i++)
	{
		dPtr = GetLinePtr(i);
...
BOOL CDIB::SwitchFromFour(CDIB& dib)
{
	...
	for (i = 0; i < 16; i++)
	{
		cols[i] = (UCHAR)ClosestColor(dib.m_pRGB+i);
	}
	for (i = 0; i < h; i++)
	{
		dPtr = GetLinePtr(i);
...
BOOL CDIB::SwitchFrom24(CDIB& dib)
{
	...
	for (i = 0; i <= 0x7FFF; i++)
	{
		rgb.rgbRed = (UCHAR)((((i & 0x7C00)>>10) << 3) | 0x07);
		rgb.rgbGreen = (UCHAR)((((i & 0x3e0)>>5) << 3) | 0x07);
		rgb.rgbBlue = (UCHAR)(((i & 0x1F)<<3) | 0x07);
		index_ptr[i] = (UCHAR)ClosestColor(&rgb);
	}
	for (i = 0; i < h; i++)
	{
		dPtr = GetLinePtr(i);
